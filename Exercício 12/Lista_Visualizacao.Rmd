...
barplot(values,
        ...
        col=colors,
        beside = F)
legend("topright", pch=c(15,15,15), col=colors, legend=categories)
```

## Questão 3
```{r echo=TRUE, error=F, message=FALSE, warning=FALSE}
library(dplyr)
data = airquality %>% filter(Month==5) %>% select(Temp, Day) # Seleciona as colunas temp e day do mês de maio
data$Temp_C = (data$Temp-32)/1.8 #Adiciona uma nova coluna com temperatura em Celsius
dens = density(data$Temp_C) #Curva de densidade
hist(data$Temp_C,
     col="grey",
...
pct <- round(sales$SALES/sum(sales$SALES)*100) # Calcula porcentagem de vendas
